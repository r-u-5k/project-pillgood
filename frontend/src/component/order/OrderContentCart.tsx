...
export const OrderContentCart = () => {
  ...
  useEffect(() => {
    ...
    fetchDefaultAddress();
    setSelectedCart(selectedItems);
    const cartItemIds = selectedItems.map((item) => item.cartId);
    const totalPrice = calculateTotalPrice(selectedItems) + 3000;
    setOrder((prevOrder) => ({
      ...prevOrder,
  ...
  const calculateTotalPrice = (selectedCart) => {
    return selectedCart.reduce((total, item) => {
      const itemPrice = item.cartQty * item.itemDto.price;
      return total + itemPrice;
    }, 0);
  };
  console.log(calculateTotalPrice(selectedCart));
  ...
  const payButton = () => {
    ...
    if (!phoneRegex.test(order.ordersPhone)) {
      toast.error("유효하지 않은 연락처 입니다.");
      formRef.current.ordersPhone.focus();
      return;
    }
        ...
        <div className="w-full h-full">
          ...
          >
            <div>
              ...
              <h2 className="text-[25px] mt-[30px]">제품 정보</h2>
              <div className="w-full h-full flex-col mt-[30px]">
                {selectedCart.map((item) => (
                  <div className="w-full h-[65px] mb-[20px] flex flex-row">
                    ...
                    ></input>
                    <img
                      src={item.itemDto.itemImageDto[0].url}
                      className="w-[65px] bg-gray-200 rounded-xl"
                    />
                    <div className="flex flex-col ml-[15px]">
                      <div>
                        ...
                        >
                          {item.itemDto.categoryType}
                        </Badge>
                      </div>
                      <span>
                        {`[${item.itemDto.brand}] ${item.itemDto.name}`}{" "}
                      </span>
                      <span>
                        {" "}
                        {item.cartQty} 개{" "}
                        {(
                          Number(item.cartQty) * Number(item.itemDto.price)
                        ).toLocaleString()}
                        원
                      </span>
                    </div>
                ...
                <div className="w-full h-[130px] border-[1px] border-gray-600 rounded-xl py-[5px] px-[25px] flex flex-col mt-[50px]">
                  <div className="w-full h-[40px] flex justify-between">
                    <div className="flex justify-start items-center">
                      제품합계금액
                    </div>
                    <div className="flex justify-end items-center">
                      {calculateTotalPrice(selectedCart).toLocaleString()}원
                    </div>
                  </div>
                  <div className="w-full h-[40px] flex justify-between">
                    <div className="flex justify-start items-center">
                  ...
                  <div className="w-full h-[40px] flex justify-between">
                    ...
                    <div className="flex justify-end items-center">
                      {(
                        calculateTotalPrice(selectedCart) + 3000
                      ).toLocaleString()}
                      원
                    </div>
                  </div>
              ...
              <div className="w-full h-[40px] flex justify-between">
                ...
                <Button
                  size="sm"
                  className="ml-[10px] rounded-xl bg-gray-900 hover:bg-gray-900 text-white font-semibold w-[130px] h-[45px]"
              ...
              <div className="mt-[30px]">
                ...
                <Input
                  type="text"
                  placeholder="수령인 연락처(-빼고 숫자만)를 입력해 주세요."
                  maxLength={11}
                  className="mt-[10px] h-[45px] rounded-xl border-[1px] focus:border-[2px]"
                  name="ordersPhone"
                  value={order.ordersPhone}
                  onChange={handleChangeForm}
                />
              </div>
              <div className="mt-[30px]">
              ...
              <div className="mt-[30px]">
                ...
                <Input
                  ...
                  className="mt-[10px] h-[45px] rounded-xl border-[1px] focus:border-[2px]"
                  name="ordersAddressDetail"
                  value={order.ordersAddressDetail}
                  onChange={handleChangeForm}
                />
              </div>
              <div className="mt-[30px]">
